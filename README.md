...
| ⌨️ | 热键绑定 | 为信息遮罩、奇遇计数、自动战斗和视角巡航配置快捷切换。 |
...
3. 在首页点击“启动”时会从游戏画面识别 UID；匹配已有账号后，本次任务的自动统计会固定记录到该账号。新 UID 会在应用和信息遮罩顶部同时提示，并要求核对确认。
4. 识别到当前赛季的奇遇解除信号后，工具会自动更新当前启动账号的计数。
5. 可在“统计”页管理账号，手动新增、编辑或删除奇遇和异色记录。
6. 识别到异色提示后，请前往“统计”页确认结果；确认后会写入异色记录，并重置对应精灵的当前赛季计数。
7. 需要备份统计数据时，可在“统计”页导入或导出数据；需要多设备同步时，可设置 Cloudflare R2 云同步，配置说明见 [Cloudflare R2 同步教程](docs/statistics-sync-cloudflare-r2.md)。
...
在“热键”页可以为信息遮罩、奇遇计数、自动战斗和视角巡航设置快捷切换。视角巡航默认绑定 `P`，可点击按钮重新录入按键，设置时按 `Esc` 可清除绑定。热键仅在目标游戏窗口为焦点时触发。视角巡航的移动距离、移动间隔、换向间隔和起始方向可在“设置”页调整并保存。
...
- 图像识别素材分别以 `2048×1152`（16:9）和 `1920×1440`（4:3）制作，理论上支持这两种画面比例下的其他分辨率；若识别效果不佳，建议切换至对应的素材基准分辨率，并检查游戏 UI 缩放设置。
